...
const EditCoupon = ({ data, close, fetchData }) => {
  ...
  const handleUpdate = async () => {
    ...
    try {
      const response = await Axios({
        ...SummaryApi.coupon_update(data._id),
        data: form
      });

      const { data: responseData } = response;
      if (responseData.success) {
        toast.success(responseData.message || "Coupon updated");
        fetchData();
        close();
      } else {
      ...
      <div className='bg-white rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden'>
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
            <div className='flex flex-col gap-1'>
              ...
              <input 
                ...
                placeholder="SAVE50" 
                className='p-2.5 border rounded-xl focus:ring-2 focus:ring-blue-500 outline-none uppercase bg-gray-50' 
              />
            </div>
            ...
            <div className='flex flex-col gap-1'>
              ...
              <input 
                ...
                placeholder="10" 
                type="number" 
                className='p-2.5 border rounded-xl focus:ring-2 focus:ring-blue-500 outline-none' 
              />
            </div>
            ...
            <div className='flex items-center gap-6 md:col-span-2 mt-2'>
              <label className='flex items-center gap-2 cursor-pointer group'>
                ...
                <span className='text-sm font-medium text-gray-700 group-hover:text-blue-600 transition'>First Order Only</span>
              </label>
              ...
              <label className='flex items-center gap-2 cursor-pointer group'>
                ...
                <span className='text-sm font-medium text-gray-700 group-hover:text-blue-600 transition'>Active</span>
              </label>
            </div>
          </div>
        </div>
        ...
        <div className='flex gap-4 justify-end p-6 border-t bg-gray-50'>
          ...
          <button 
            ...
            className='px-8 py-2.5 rounded-xl bg-blue-600 text-white font-bold hover:bg-blue-700 shadow-lg shadow-blue-200 transition active:scale-95'
          >
            Update Coupon
          </button>
        </div>
