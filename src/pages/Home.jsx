...
const Home = () => {
  ...
  return (
    <section className="bg-white dark:bg-neutral-950 min-h-screen transition-colors duration-300">
      ...
      {loadingOffers ? (
        <div className="container mx-auto px-4 mt-8 lg:mt-12 relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
             {[1, 2, 3].map(n => (
                <div key={n} className="aspect-21/9 md:aspect-video rounded-3xl bg-gray-50/50 border border-gray-100 shadow-xs animate-pulse" />
             ))}
          </div>
        </div>
      ) : (
      ...
      {loadingOffers ? (
        <div className="mt-8 lg:mt-12 bg-slate-50/70 dark:bg-neutral-900/20 py-6 border-y border-slate-100 dark:border-neutral-900">
           <div className='container mx-auto px-4 pb-4 flex items-center justify-between'>
              <Skeleton className="h-8 w-80 rounded-lg" />
           </div>
           <div className='flex gap-4 mb-3 md:gap-6 lg:gap-8 container mx-auto px-4 overflow-x-hidden'>
              {[1, 2, 3, 4, 5, 6].map(n => (
                 <ProductLoading key={"DealsLoading" + n} styleWidth="w-44 sm:w-52 lg:w-60 xl:w-64" />
              ))}
           </div>
        </div>
      ) : (
      ...
      {loadingOffers ? (
        <div className="mt-8 lg:mt-12 bg-white dark:bg-neutral-950 py-6">
           <div className='container mx-auto px-4 pb-4 flex items-center justify-between'>
              <Skeleton className="h-8 w-56 rounded-lg" />
           </div>
           <div className='flex gap-4 mb-3 md:gap-6 lg:gap-8 container mx-auto px-4 overflow-x-hidden'>
              {[1, 2, 3, 4, 5, 6].map(n => (
                 <ProductLoading key={"NewArrivalsLoading" + n} styleWidth="w-44 sm:w-52 lg:w-60 xl:w-64" />
              ))}
           </div>
        </div>
      ) : (
